...
//            if (current->proc (current, MSG_MOUSE_DOWN, 0) == RET_CALL && current->call) {
            if (current->call && current->proc (current, MSG_MOUSE_DOWN, 0) == RET_CALL) {
    ...
    else if (state == RET_REDRAW) {
        state = 0;
        if (object_focus && object_focus->visible && object_focus->focus) {
            app_ObjectUpdate (object_focus);
        }
    }
}// app_UpdateGui ()
...
//    SDL_FillRect (screen, &(SR){ 1, hh, screen->w-1, hh }, COLOR_BLUE2);
    ...
    SDrawText (screen, "To Exit Press The KEY: ESC", 100, (screen->h/2), COLOR_WHITE);
}
