...
char *app_ConsoleGetText (OBJECT *o) {
...
void app_ConsoleSetText (OBJECT *o, char *text) {
    DATA_CONSOLE *data = app_GetData(o);
    if (data && text) {
        int len = strlen(text);
        if (len < CONSOLE_TEXT_SIZE) {
    ...
    case MSG_DRAW: {
        ...
        while (iten) {
          ...
          break;
            ...
            SDrawText (screen, sbuf, r.x+10, pos_y+10, COLOR_ORANGE);
            while (*s) {
        ...
        //SDrawText (screen, data->text, r.x+10, r.y+r.h-21, COLOR_GREEN);
        DrawRectR (screen, r.x, r.y, r.w, r.h, COLOR_ORANGE); // border
        sprintf (sbuf, "%d", data->count);
        SDrawText (screen, sbuf, (r.x+r.w)-(strlen(sbuf)*8)-10, r.y+10, COLOR_ORANGE);
    ...
    case MSG_KEY:
      ...
      return 0;
        ...
        if (value == SDLK_UP) {
            ...
            if (data->text[0] == '$') {
                iten_color = app_ConsoleAdd (o, data->text, COLOR_GREEN);
...
ITEN * app_ConsoleAdd (OBJECT *o, char *text, int color) {
    DATA_CONSOLE *data = app_GetData(o);
    ITEN *iten;
...
void app_ConsoleTOP (OBJECT *o) {
    DATA_CONSOLE *data = app_GetData(o);
    if (data && data->top < data->count-1) {
        data->top++;
        data->iten_top = data->iten_top->next;
    }
}
...
static void thanks (OBJECT *o) {
    ...
    app_ConsoleAdd (o, "  01: The eternal, the creator of the heavens and the earth.", COLOR_WORD);
