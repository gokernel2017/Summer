...
*/
    ...
    case MSG_DRAW: {
          ...
          break;
                  ...
                  if (isoperator(*str))
                    ...
                    if (!is_reserved_word && !iswordchar(str[-1])) {
                        ...
                        if ((*str >= 'a' && *str <= 'g') || *str=='i' || *str=='l' || *str=='o' || (*str >= 'r' && *str <= 'v') || *str=='w') {
                            ...
                            int count = 0, i;
                            char word [20];
                            char *WORDS[] = { "asm", "break", "case","char","const","continue","default","do","double","else","enum","extern","float","for","goto","if","int","long","register","return","short","signed","sizeof","static","struct","switch","typedef","union","unsigned","void","volatile","while",
            ...
            if (*str == '\n') { // <-- New line
                ...
                sprintf (buf, "%04d", data->line_top + i); i++;
								SDrawText (screen, buf, r.x+4, pos_y, COLOR_ORANGE);
        ...
        if (data->saved)
            SDrawText (screen, buf, r.x+5, r.y+r.h-15, COLOR_WHITE);
        else
            SDrawText (screen, buf, r.x+5, r.y+r.h-15, C_COMMENT);
...
OBJECT * app_NewEditor (OBJECT *parent, int x, int y, char *text, int size) {
    OBJECT *o;
    DATA_EDITOR *data;
...
*/
    ...
    o = app_ObjectNew (proc_editor, x, y, 320, 245, 0, OBJECT_TYPE_EDITOR, data);
